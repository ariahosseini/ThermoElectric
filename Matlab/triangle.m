...
N       =   v_frac/l(3)/(l(1)*l(2)/2);              % number of particles in unit volume
%% define kpoints
...
% find matrix element
if kind == 3
M              =  2*uo*(-1*((exp(l(1)*qx*1i)*1i - 1i)*1i)./(qx.*qy) ...
                 +(l(1)*(exp(l(1)*qx*1i).*exp(l(2)*qy*1i)*1i - 1i)*1i)./...
                        (qy.*(l(1)*qx + l(2)*qy))).*(sin(l(3)*qz/2)./qz);

elseif kind == 2
M              =  2*uo*((-((exp((l(1)*qx*1i)/2)*1i - 1i)*1i)./(qx.*qy)+...
...
elseif kind ==1
M              =  2*uo*(-(qx.*(l(1) - l(1).*exp(l(2)*qy*1i))- ...
